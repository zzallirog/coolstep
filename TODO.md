...
## Phase P3.0 — Rust/C++ HNSW backend (🚨 TRIGGERED 2026-05-13)

Operator framing: cockpit refresh-health (shipped 2026-05-13) exposed
chronic `predict_refresh_last_ms=9466ms` + `predict_refresh_skipped=78`
at 42k-vector chroma index. At 10Hz tick this means ~95 ticks reuse
stale prediction per refresh → trail freezes, "иногда подвисает".

- [x] **Phase 1 — proof of life** ✅ DONE 2026-05-13
      → `~/.claude/work/coolstep-rust-hnsw/HANDOFF.md`
      - [x] `core/knn_hnsw.py` (hnswlib backend, same public surface)
      - [x] `core/knn.py` backend selector via `COOLSTEP_KNN_BACKEND`
      - [x] `scripts/migrate_chroma_to_hnsw.py` one-shot bootstrap
      - [x] tests/test_knn_hnsw.py — 10 new tests, 849 total pass
      - **Synthetic 42k benchmark:** query p50 0.57ms / p95 0.78ms
        (vs 9466ms chroma; 128× under the 100ms gate). Upsert 5026/s.
- [ ] **Phase 2 — dual-write soak** (7 days)
      - `COOLSTEP_KNN_DUAL=1`: write both, query both, log Jaccard
      - Refresh-health tile gains `hnsw_p50_ms`, `jaccard_p50`
      - Gate: 7d window with Jaccard p50≥0.95, p95≥0.85
- [ ] **Phase 3 — flip** (1 session after soak gate)
      - Set `COOLSTEP_KNN_BACKEND=hnsw` in drop-in
      - One more week dual-write, then drop chroma reads
      - 30 days later: archive chroma dir, demote chromadb dep to extras

Acceptance: `predict_refresh_last_ms` <100ms p95, `predict_refresh_skipped`
stops growing in steady-state, residual_trail no longer freezes.

Memory: `[[project-coolstep-hnsw-todo]]` (filed 2026-05-12, triggered 2026-05-13).
...
### P2.9.4 — Dual-curve canvas ✅ DONE 2026-05-13 (v0.5.7)
- [x] Hardware-current solid teal Newton τ=4s alongside dashed
      KNN-anchored historical envelope.  Gap = installed margin.
- [x] Legend swatches added (archive forecast · hw-current).
- [x] Respects active horizon from P2.9.3.

### P2.9.5 — Dashboard softness ✅ DONE 2026-05-13 (v0.5.7+v0.5.8)
- [x] Self-rescheduling `setTimeout` (no setInterval drift)
- [x] Canvas redraw signature-gated (only when data changes)
- [x] rAF tween on hero metrics — 400ms cubic glide (v0.5.8)
- [x] Pin lifetime tied to active horizon (v0.5.8)

### P2.9.6 — Daemon tick throughput ✅ DONE 2026-05-13 (v0.5.7)
- [x] DEFAULT_PERIOD 1.0 → 0.1 (10Hz)
- [x] Predict cache (3s refresh, async refill)
- [x] chroma.add background queue + drain worker
- [x] chroma.count / count_labeled / dir_size_bytes cached
- [x] Periodic ops scaled via ticks_per_sec
- [x] posthog telemetry wrapper monkey-patched (silenced)

### P2.9.7 — Confidence ≠ accuracy split (planned, NOT STARTED)
- [ ] Cockpit now shows single confidence pill that conflates two
      different signals: coverage («how many neighbours did we
      find?») vs accuracy («how often was the archive right?»).
- [ ] Surface BOTH metrics side-by-side:
  - **COVERAGE** — current `confidence` (1.0 = 20/20 neighbours in
    bucket, 0.5 = sparse archive)
  - **ACCURACY** — derived from `accuracy_pct` (already in API),
    rolling median |residual| → 0..1 mapping
- [ ] Layout: two acc-chips side-by-side, similar to ±err pills
- [ ] Tooltip: coverage = «how much archive supports this prediction»,
      accuracy = «how often the archive was right»
- *Why: operator 2026-05-13 «на счет конфиденциальности не знаю» —
  confidence shows 1.0 while residuals are ±28°C; the conflation
  misleads.  Backend already returns accuracy_pct; just expose in UI.*
...
### P2.8.0 — Warm-start reindex ✅ DONE 2026-05-13 (v0.5.6)
- [x] `scripts/reindex_chroma_from_store.py` shipped — 42k vectors
      reindexed; embedder-stats.json persistence prevents drift.
