...
# Drop-in templates intentionally omit `Slice=` so the unit defaults to
# `user.slice` (always present). Users opt into a custom slice via their own
# higher-numbered drop-in. See scripts/dropin-templates/laptop.conf comments.

# --------------------------------------------------------------------------- #
# T1: laptop profile — CPUQuota + MemoryMax + flag echo
# --------------------------------------------------------------------------- #
OUT=$(bash "$INIT" --dry-run --profile laptop 2>&1)
_assert_contains "T1.1 laptop: CPUQuota=10%"          "CPUQuota=10%"       "$OUT"
_assert_contains "T1.2 laptop: MemoryMax=512M"        "MemoryMax=512M"     "$OUT"
_assert_contains "T1.3 laptop: profile printed"       "profile=laptop"     "$OUT"
_assert_contains "T1.4 laptop: dry-run flag echoed"   "dry-run=true"       "$OUT"
_assert_not_contains "T1.5 laptop: no Slice= leak"    "Slice="             "$OUT"

# --------------------------------------------------------------------------- #
# T2: desktop profile — CPUQuota + MemoryMax
# --------------------------------------------------------------------------- #
OUT=$(bash "$INIT" --dry-run --profile desktop 2>&1)
_assert_contains "T2.1 desktop: CPUQuota=30%"  "CPUQuota=30%"      "$OUT"
_assert_contains "T2.2 desktop: MemoryMax=1G"  "MemoryMax=1G"      "$OUT"
_assert_contains "T2.3 desktop: profile echo"  "profile=desktop"   "$OUT"
_assert_not_contains "T2.4 desktop: no Slice= leak" "Slice="       "$OUT"
