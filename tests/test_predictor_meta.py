...
def test_meta_applies_learned_correction():
    """After observing 30× -10°C residuals in a bucket, the next
    prediction at that bucket should be ~10°C lower than base."""
    base = TrajectoryBaseline()
    bank = ResidualBank()
    features = {
    ...
    base_pred = base.predict(features, [])
    meta_pred = meta.predict(features, [])
    # Correction roughly -10°C
    assert -11.0 <= meta_pred.expected_temp_c - base_pred.expected_temp_c <= -9.0
