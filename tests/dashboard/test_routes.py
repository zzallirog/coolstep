...
# Balance-plan step III: /api/hot — mmap hot-state pass-through.

def test_hot_endpoint_unavailable_when_no_mmap(client, tmp_path, monkeypatch):
    """No state.mmap file → endpoint returns {available: false}, HTTP 200."""
    # client fixture uses default XDG_RUNTIME_DIR; point hot_state at a clean
    # tmp dir so the live target's file (if any) doesn't bleed in.
...
def test_hot_endpoint_reads_written_state(client, tmp_path, monkeypatch):
    """Writer writes a record → endpoint surfaces every field with age_sec
    bounded (the field exists, is non-negative, and is small for a fresh
    write). All scalars round-trip through the mmap layout."""
    monkeypatch.delenv("RUNTIME_DIRECTORY", raising=False)
    monkeypatch.setenv("XDG_RUNTIME_DIR", str(tmp_path / "xdg"))
    from coolstep.core import hot_state as _hot
    path = _hot.default_path()
    w = _hot.Writer(path)
    try:
        w.write(_hot.HotState(
            ts_sec=0.0,  # ignored — writer stamps wall clock
            cpu_temp_now=71.5,
            gpu_temp_max=55.0,
            fan_max_rpm=2200.0,
            throttle_prob=0.08,
            expected_temp_c=73.0,
            busy_ratio_ewma=0.21,
        ))
    finally:
        w.close()
    ...
    assert body["cpu_temp_now"] == 71.5
    assert body["gpu_temp_max"] == 55.0
    assert body["fan_max_rpm"] == 2200.0
    assert body["throttle_prob"] == 0.08
    assert body["expected_temp_c"] == 73.0
    assert body["busy_ratio_ewma"] == 0.21
    assert body["age_sec"] >= 0.0
    assert body["age_sec"] < 5.0  # fresh write


# Balance-plan step II: /api/self — observer-effect self-monitor surface.

def test_self_endpoint_shape_when_ml_state_absent(client, monkeypatch):
    """No ml-state.json + no systemctl → endpoint still returns 200 with the
    full body skeleton (every key present, every value None where unknown)."""
    import subprocess as _sp
    ...
    r = client.get("/api/self")
    assert r.status_code == 200
    body = r.json()
    assert body["tick"] is None
    assert body["memory"] == {
        "current_mb": None, "swap_mb": None, "limit_mb": None, "used_pct": None,
    }
    assert body["psi"] == {
        "cpu_avg10": None, "memory_avg10": None, "io_avg10": None,
    }
    assert body["restarts"]["count"] is None
    assert body["triggers"] == []
    assert body["status"] == "ok"


def test_self_endpoint_surfaces_ml_state_self_monitor_block(
    client, tmp_path, monkeypatch,
):
    """ml-state.json with self_monitor block → endpoint reflects it under
    body['tick'], and no triggers fire when values are inside thresholds."""
    import json as _json
    import subprocess as _sp

    def _raise(*_args, **_kw):
        raise FileNotFoundError("systemctl unavailable in tests")
    monkeypatch.setattr(_sp, "run", _raise)

    # `client` fixture set COOLSTEP_HOME=tmp_path/data; write ml-state.json there.
    state_dir = tmp_path / "data"
    (state_dir / "ml-state.json").write_text(_json.dumps({
        "self_monitor": {
            "busy_ratio_ewma": 0.18,
            "busy_ratio_p95_60_ticks": 0.42,
            "tick_total_ms_p95_60_ticks": 84.0,
            "slow_tick_count_60_ticks": 0,
            "slow_tick_threshold_ms": 500.0,
            "ring_samples": 60,
        },
    }))
    ...
    assert r.status_code == 200
    body = r.json()
    assert body["tick"]["busy_ratio_ewma"] == 0.18
    assert body["tick"]["busy_ratio_p95_60_ticks"] == 0.42
    assert body["triggers"] == []
    assert body["status"] == "ok"


def test_self_endpoint_flags_busy_ratio_trigger(
    client, tmp_path, monkeypatch,
):
    """busy_ratio_p95 > 1.0 → 'busy_ratio_p95' appears in triggers[] and
    status flips to 'trigger'."""
    import json as _json
    import subprocess as _sp
    ...
    (state_dir / "ml-state.json").write_text(_json.dumps({
        "self_monitor": {
            "busy_ratio_ewma": 0.92,
            "busy_ratio_p95_60_ticks": 1.05,
            "tick_total_ms_p95_60_ticks": 210.0,
            "slow_tick_count_60_ticks": 4,
            "slow_tick_threshold_ms": 500.0,
            "ring_samples": 60,
        },
    }))
    ...
    assert r.status_code == 200
    body = r.json()
    trigger_ids = {t["id"] for t in body["triggers"]}
    assert "busy_ratio_p95" in trigger_ids
    assert "slow_tick_count" in trigger_ids
    assert body["status"] == "trigger"


def test_debug_trim_endpoint(client):
    """malloc_trim + gc.collect smoke. Linux-only environment expected;
    `called=False` is a valid response on non-glibc/non-Linux runners.

    Regression context: 2026-05-16 measured RSS 302M → 207M on prod
    dashboard via single trim call (glibc holds free chunks until trim
    walks the top). The lifespan task runs the same operation every
    `_MALLOC_TRIM_INTERVAL_SEC` to keep RSS bounded.
    """
    r = client.get("/api/debug/trim")
    assert r.status_code == 200
    body = r.json()
    assert "called" in body
    assert "rc" in body
    assert "rss_kb_before" in body
    assert "rss_kb_after" in body
    if body["called"]:
        # glibc returns 1 if memory was released, 0 otherwise — both are valid.
        assert body["rc"] in (0, 1)
