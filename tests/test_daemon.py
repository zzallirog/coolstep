...
def test_control_apply_records_intervention_window(tmp_path, monkeypatch):
    ...
    assert daemon._interventions_overlapping(95.0, 105.0) == ("ramp_cooling",)
    assert daemon._interventions_overlapping(
...
def test_intervened_validation_does_not_train_residual_bank(tmp_path, monkeypatch):
    """Residuals whose horizon overlaps hardware control stay logged but do
    not feed MetaPredictor.bank."""
    from coolstep.core.predictor import TrajectoryBaseline
    from coolstep.core.predictor_meta import MetaPredictor
    from coolstep.daemon import _InterventionWindow, _PendingPrediction

    daemon = _daemon_with_home(tmp_path, monkeypatch)
    daemon.predictor = MetaPredictor(base=TrajectoryBaseline())
    features = {
    ...
    bucket_key = daemon.predictor.bucket(features)
    daemon._pending_predictions = [_PendingPrediction(
        predicted_at=100.0,
        predicted_temp_c=72.0,
        model_name="trajectory_baseline+meta",
        bucket_key=bucket_key,
        features=features,
    )]
    daemon._intervention_windows.append(_InterventionWindow(
        started_at=101.0,
        ended_at=120.0,
        verb="ramp_cooling",
    ...
    daemon._validate_pending_predictions(
        now=106.0,
        current_actual=68.0,
        current_features=features,
    )

    record = daemon.residual_log.tail(1)[0]
    assert record.intervened is True
    assert record.intervention_verbs == ("ramp_cooling",)
    assert len(daemon.predictor.bank) == 0
...
def test_startup_recovery_marks_live_armed_as_intervention(tmp_path, monkeypatch):
    """A still-live recovered control action must keep residuals controlled
    ...
    assert d._interventions_overlapping(110.0, 120.0) == ("ramp_cooling",)
