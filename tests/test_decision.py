...
def _pred(prob: float, conf: float = 0.9, expected: float = 85.0) -> Prediction:
    return Prediction(
        horizon_sec=5.0,
        throttle_prob=prob,
...
def test_ramp_cooling_blocked_when_already_cooling_to_forecast():
    """If live slope is cooling and forecast is not above current Tctl,
    do not stack another anticipatory fan ramp on top of the recovery edge."""
    engine = DecisionEngine()
    actions = engine.decide(
        _pred(0.95, expected=80.2),
        calibration_ready=True,
        now=100.0,
        labeled_count=_WARM_KNN,
        cpu_temp_c=80.0,
        cpu_temp_slope_c_per_sec=-0.2,
    )
    verbs = {a.verb for a in actions}
    assert ActionVerb.RAMP_COOLING not in verbs
    assert ActionVerb.CAP_BOOST in verbs
...
def test_ramp_cooling_allowed_when_cooling_but_forecast_still_rises():
    engine = DecisionEngine()
    actions = engine.decide(
        _pred(0.95, expected=81.0),
        calibration_ready=True,
        now=100.0,
        labeled_count=_WARM_KNN,
        cpu_temp_c=80.0,
...
def test_p22_new_threshold_defaults_unchanged_for_legacy_fields():
    ...
    assert t.ramp_cooling_temp_margin_c == 0.5
    assert t.ramp_cooling_cooling_slope_c_per_sec == -0.05
...
def test_decide_appends_to_decisions_log(tmp_path, monkeypatch):
    ...
    assert rec["thresholds"]["ramp_cooling_temp_margin_c"] == pytest.approx(0.5)
