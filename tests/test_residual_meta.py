...
def test_from_log_skips_intervened_records_by_default(tmp_path):
    """Controlled residuals are visible in the log but must not train the
    passive correction bank unless explicitly requested."""
    from coolstep.core.residual_log import ResidualLog, ResidualRecord

    log = ResidualLog(tmp_path / "residual-state.jsonl")
    features = {
        "cpu_load_slope_per_sec": 0.0,
        "cpu_temp_slope_per_sec": 0.0,
        "cpu_temp_accel_per_sec_sq": 0.0,
    ...
    log.append(ResidualRecord.from_pair(
        ts=1000.0,
        predicted_at=970.0,
        horizon_sec=30.0,
    ...
    passive_bank = ResidualBank.from_log(log)
    all_bank = ResidualBank.from_log(log, include_intervened=True)

    assert next(iter(passive_bank.stats.values())).n == 1
    assert next(iter(all_bank.stats.values())).n == 2
