...
def test_supports_cache_reuses_within_5s(monkeypatch):
    """Two supports() calls within 5s share one systemctl subprocess call."""
    monkeypatch.delenv("COOLSTEP_GAME_MODE_DEFER", raising=False)
    a = AsusctlFanCurve()
    with patch("subprocess.run",
        ...
        a.supports(ActionVerb.RAMP_COOLING)
    # Only ONE systemctl probe within the 5s cache window
    assert mock_run.call_count == 1


def test_supports_cache_expires_after_5s(monkeypatch):
    """Cache TTL expiry causes a fresh systemctl call on the second probe."""
    monkeypatch.delenv("COOLSTEP_GAME_MODE_DEFER", raising=False)
    a = AsusctlFanCurve()
    t0 = 1000.0
    ...
    with patch(f"{_module}.subprocess.run",
        ...
        # First call: fills cache at t0
        a.supports(ActionVerb.RAMP_COOLING)
        # Second call: monotonic returns t0+6 -> cache stale -> re-probe
        a.supports(ActionVerb.RAMP_COOLING)
    assert mock_run.call_count == 2
