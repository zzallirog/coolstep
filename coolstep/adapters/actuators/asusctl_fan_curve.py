...
class AsusctlFanCurve:
    ...
    def _is_game_mode_active(self) -> bool:
        """Return True if game-mode.service is active right now.

        Result is cached for `_GM_CACHE_TTL_S` seconds to avoid a
        `systemctl` fork on every daemon tick. Any error or timeout is
        treated as «not active» so coolstep never blocks on a missing unit.
        Logs once per cache-window when the state flips to active.
        """
        now = time.monotonic()
        ...
        try:
            ...
            is_active = cp.returncode == 0 and cp.stdout.strip() == b"active"
        except Exception:  # noqa: BLE001
            is_active = False

        if is_active and not prev_active:
            log.debug("game-mode active, supports() -> False (deferring)")
