...
def _sudoers_preflight() -> bool:
    """Probe that `sudo -n ryzenadj --version` works (NOPASSWD entry present).

    Returns True if the sudoers snippet documented in this module's
    top-level docstring is installed. False on rc!=0 stderr "password is
    required" (sudoers entry missing) or any other failure. Used by make()
    so we don't load an actuator that will silently return rc=1 on every
    apply() call.
    """
    ...
    try:
        cp = subprocess.run(
            ["sudo", "-n", "/usr/bin/ryzenadj", "--version"],
    ...
    if cp.returncode == 0:
        return True
    stderr = (cp.stderr or b"").decode("utf-8", errors="replace").lower()
    if "password is required" in stderr or "no tty" in stderr:
        log.warning(
            "ryzenadj_cap: sudoers preflight failed — install NOPASSWD entry: "
            "printf '%%s ALL=(root) NOPASSWD: /usr/bin/ryzenadj\\n' \"$USER\" "
            "| sudo tee /etc/sudoers.d/coolstep-ryzenadj && sudo chmod 0440 "
            "/etc/sudoers.d/coolstep-ryzenadj"
        )
    return False
...
def make() -> RyzenadjCap | None:
    ...
    _c = caps_if_set()
    if _c is not None:
        if not _c.ryzenadj_available:
            return None
        # Interference gate (S04): TLP also writes STAPM/PPT MSRs via WriteACPI
        # on AMD systems. Ceding when TLP is active avoids fight on the same
        # limits. Same kill-switch convention as EPP shift.
        force = os.environ.get("COOLSTEP_RYZENADJ_FORCE", "0").lower() in {"1", "true"}
        if not force and _c.tlp_active:
            log.info("ryzenadj_cap: tlp.service active — ceding STAPM/PPT ownership (set COOLSTEP_RYZENADJ_FORCE=1 to override)")
            return None
    if not _host_supported():
        return None
    # Sudoers preflight (S16): apply() shells out via `sudo -n`. If the
    # NOPASSWD snippet is missing, every apply returns rc=1 forever — and
    # nothing notifies the user. Probe once at startup; refuse to load if
    # the entry isn't there, so the install_plan can surface the gap.
    if not _sudoers_preflight():
        return None
    return RyzenadjCap()
