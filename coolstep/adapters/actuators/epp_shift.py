...
def make() -> EppShift | None:
    ...
    _c = caps_if_set()
    if _c is not None:
        if not _c.epp_available:
            return None
        # Interference gate (S03/S04 in interference-matrix.md):
        # power-profiles-daemon and TLP both write to
        # /sys/.../cpufreq/energy_performance_preference. If either is the
        # active EPP owner, ceding the surface avoids a silent write race.
        # Override with COOLSTEP_EPP_SHIFT_FORCE=1 if user knows what they're
        # doing (e.g. PPD locked into a profile they don't want for cooling).
        force = os.environ.get("COOLSTEP_EPP_SHIFT_FORCE", "0").lower() in {"1", "true"}
        if not force:
            if _c.power_profiles_daemon_active:
                log.info("epp_shift: power-profiles-daemon active — ceding EPP ownership (set COOLSTEP_EPP_SHIFT_FORCE=1 to override)")
                return None
            if _c.tlp_active:
                log.info("epp_shift: tlp.service active — ceding EPP ownership (set COOLSTEP_EPP_SHIFT_FORCE=1 to override)")
                return None
