...
export class LiveTelemetryTile extends LitElement {
  ...
  connectedCallback() {
    ...
    this._es.onerror = () => {
      this.error = 'SSE disconnected — auto-reconnect…';
    };
    this._refreshRaw();
    this._rawTimer = setInterval(() => this._refreshRaw(), 3000);
  ...
  render() {
    ...
    const tempCls = tctl == null ? '' : tctl >= 90 ? 'err' : tctl >= 80 ? 'warn' : tctl >= 65 ? 'cat' : 'ok';
      ...
      body: html`
        ...
        <div class="hero">
          <div class="col">
            ...
            <span class="metric ${tempCls}">${fmtNum(tctl, 1)}<small>°C</small></span>
          </div>
          <div class="aside">
        ...
        <div class="mini-grid">
          <div class="mini">
            ...
            <span class="v">${fmtNum(f.gpu_temp, 1)}<small>°C</small></span>
          </div>
          <div class="mini">
            <span class="eyebrow">CPU power</span>
            <span class="v">${fmtNum(f.cpu_power, 1)}<small>W</small></span>
          </div>
          <div class="mini">
            <span class="eyebrow">GPU power</span>
            <span class="v">${fmtNum(f.gpu_power, 1)}<small>W</small></span>
          </div>
          <div class="mini">
            <span class="eyebrow">Fan max</span>
            <span class="v">${fmtNum(f.fan_max_rpm, 0)}<small>rpm</small></span>
          </div>
        </div>
