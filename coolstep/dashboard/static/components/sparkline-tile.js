...
export class SparklineTile extends LitElement {
  static styles = [
    ...
    css`
      ...
      /* G-7 mirror — italic dim N/A when sensor structurally absent on this
       * platform (Intel iGPU has no discrete temp/power, no fan tool, etc).
       * Hover for the per-signal reason. */
      .row .value.na {
        color: var(--fg-dim);
        font-style: italic;
        font-size: 14px;
  ...
  _draw(slot, series, color, opts = {}) {
    ...
    if (data.length < 2) {
      // G-7 mirror — italic "N/A" inside the sparkline body when the field
      // is structurally absent on this platform.  Aligns visually with the
      // right-hand value cell which also flips to italic N/A.
      ctx.fillStyle = '#58627c';
      ctx.font = 'italic 11px "Inter", sans-serif';
      ctx.fillText('N/A', w / 2 - 10, h / 2 + 4);
      return;
    }
  ...
  _renderRow(label, slot, field, unit, digits) {
    ...
    // G-7 mirror of live-telemetry-tile.  Same naReasons dict — keep in sync
    // when adding a new signal there.  Backlog: collector-emitted
    // `unavailable_reason` per signal so backend owns the explanation.
    const naReasons = {
    ...
    return html`
      <div class="row">
        ...
        ${hasValue
          ? html`<span class="value">${fmtNum(v, digits)}<small>${unit}</small></span>`
          : html`<span class="value na" title=${naReasons[field] || 'unavailable on this platform'}>N/A</span>`}
      </div>
    `;
  }
