...
def create_app() -> FastAPI:
    ...
    async def predictor_breakdown() -> JSONResponse:
        ...
        PAST_KNEE_C = 85.0
        trajectory_prob = 0.0
        if cpu_temp_max >= HOT_NOW_C and slope >= FAST_SLOPE:
            trajectory_prob = 0.9
        elif cpu_temp_max >= HOT_NOW_C and slope >= MED_SLOPE:
            trajectory_prob = 0.7
        elif cpu_temp_max >= PAST_KNEE_C:
    ...
    async def predictor_cockpit() -> JSONResponse:
        ...
        # Current snapshot for the cockpit center.
        current = {
            "t": float(features.get("cpu_temp_max") or 0.0),
            "slope": float(features.get("cpu_temp_slope_per_sec") or 0.0),
    ...
    async def get_mode() -> JSONResponse:
        ...
        if ml.exists():
            try:
                ...
                body["cpu_temp_c"] = feats.get("cpu_temp_max")
