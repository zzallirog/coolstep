...
Schema (one JSON object per line, NDJSON):

    {
      ...
      "intervened": false,             # true when an actuator overlapped the horizon
      "intervention_verbs": [],        # e.g. ["ramp_cooling"]
...
class ResidualRecord:
    ...
    intervention_verbs: tuple[str, ...] = field(default_factory=tuple)
    ...
    def from_pair(
        ...
        *,
        ...
        intervened: bool = False,
        intervention_verbs: tuple[str, ...] | list[str] | None = None,
    ) -> ResidualRecord:
        return cls(
            ts=ts,
            ...
            intervention_verbs=tuple(str(v) for v in (intervention_verbs or ())),
            profile=profile,
    ...
    def to_jsonable(self) -> dict[str, object]:
        ...
        d["intervention_verbs"] = list(self.intervention_verbs)
        return d


def _intervention_verbs_from(obj: dict[str, object]) -> tuple[str, ...]:
    raw = obj.get("intervention_verbs") or ()
    if isinstance(raw, str):
        return (raw,)
    try:
        return tuple(str(v) for v in raw)  # type: ignore[union-attr]
    except TypeError:
        return ()
        ...
        for line in tail_lines:
            ...
            try:
                records.append(ResidualRecord(
                    ...
                    intervened=bool(obj.get("intervened", False)),
                    intervention_verbs=_intervention_verbs_from(obj),
