...
class AlwaysIdleBaseline:
    ...
    def predict(
        ...
        return Prediction(
            ...
            throttle_prob=0.0,
            expected_temp_c=features.get("cpu_temp_now", features.get("cpu_temp_max")),
            confidence=1.0,
...
class TrajectoryBaseline:
    ...
    def predict(
        ...
        import math
        # T0 must be the latest sample, not a rolling-window max — otherwise
        # the saturation extrapolation anchors to a past peak and the +5s
        # forecast latches at that peak until the window slides off it.
        current = features.get("cpu_temp_now", features.get("cpu_temp_max"))
        # Short slope (~5 frames) is the right input for a 5-second
        # forecast: the full-window slope (600 frames @ 1 Hz) averages
        # over ~10 min of bidirectional jitter and goes to ≈0, which made
        # predicted ≈ current at all times.
        slope = (
            features.get("cpu_temp_slope_per_sec_short")
            if features.get("cpu_temp_slope_per_sec_short") is not None
            else features.get("cpu_temp_slope_per_sec", 0.0)
...
class KnnPredictor:
    ...
    def predict(
        ...
        latest = window[-1]
        vector = self._embedder.embed(latest, features)
        if vector is None:
            return _merge_with_trajectory(
