...
from coolstep.core.workload_profile import GAME_CLASSES, RENDER_CLASSES, Profile
...
# Single source of truth: workload_headroom fires for any class that the
# profile resolver marks as Render or Game. Avoids the two-list-drift risk
# spotted during P2.5 exploration.
WORKLOAD_CLASSES_HEADROOM: frozenset[str] = RENDER_CLASSES | GAME_CLASSES
...
def workload_headroom(anchors: list[Anchor], ctx: CurveContext) -> list[Anchor]:
    ...
    if ctx.workload_class is None:
        return anchors
    cls = ctx.workload_class.lower()
    if cls not in WORKLOAD_CLASSES_HEADROOM:
        return anchors
    out: list[Anchor] = []
    for temp_c, pwm_pct in anchors:
