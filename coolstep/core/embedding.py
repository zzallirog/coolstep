...
FEATURE_NAMES: tuple[str, ...] = (
    ...
    # Trajectory features (require windowed context; 0.0 when unavailable).
    # KNN on a snapshot misses rising/falling trajectories — two states at
    # the same temperature but opposite slopes are thermally very different.
    # Source: fingerprint.extract() keys (short OLS window ~5 frames).
    "cpu_temp_slope_5s",      # °C/s  — cpu_temp_slope_per_sec_short
    "cpu_temp_accel",         # °C/s² — cpu_temp_accel_per_sec_sq
    "cpu_load_slope_5s",      # %/s   — cpu_load_slope_per_sec
)
...
def extract_raw(
    ...
    windowed_features: dict[str, float] | None = None,
) -> dict[str, float]:
    """Frame → dict of named raw scalars. Missing → 0.0 (cold start safe).

    windowed_features: output of fingerprint.extract(window) — supplies the
    three trajectory features (slope/accel).  Pass None when only a single
    frame is available (refit, incident embedding); trajectory features will
    be 0.0 and their normalization stats will use default MAD=1.0.
    """
    ...
    if windowed_features:
        out["cpu_temp_slope_5s"] = windowed_features.get("cpu_temp_slope_per_sec_short", 0.0)
        out["cpu_temp_accel"] = windowed_features.get("cpu_temp_accel_per_sec_sq", 0.0)
        out["cpu_load_slope_5s"] = windowed_features.get("cpu_load_slope_per_sec", 0.0)
...
class Embedder:
    ...
    def embed(
        ...
        if not self._fitted:
            return None
        raw = extract_raw(frame, windowed_features)
        scaled = [
