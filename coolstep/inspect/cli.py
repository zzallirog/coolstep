...
@main.command("predict-debug")
@click.option("--bucket", default=None, help="Pretty-print a single bucket (format: 'load,slope,accel,profile' as ints).")
@click.option("--top", default=10, type=int, help="Show top-N buckets by sample count.")
@click.option("--as-json", "as_json", is_flag=True, help="Emit JSON instead of human table.")
def predict_debug(bucket: str | None, top: int, as_json: bool) -> None:
    """Inspect the meta-predictor's residual bank state.

    Replays the residual log into an in-memory bank, then dumps the
    learned correction and uncertainty per bucket.  Useful before/after
    a profile flip to confirm decay worked, or to spot a bucket where
    the predictor is still wrong systematically.
    """
    from coolstep.core.residual_log import ResidualLog
    from coolstep.core.residual_meta import ResidualBank

    log = ResidualLog(_coolstep_home() / "residual-state.jsonl")
    bank = ResidualBank.from_log(log)

    if not bank.stats:
        click.echo("(empty bank — no residuals with bucket_key on disk yet)")
        return

    if bucket is not None:
        try:
            parts = tuple(int(x) for x in bucket.split(","))
        except ValueError:
            raise click.UsageError("--bucket must be 4 comma-separated ints (load,slope,accel,profile)")
        from coolstep.core.residual_meta import inv_log_residual
        stat = bank.stats.get(parts)
        if stat is None:
            click.echo(f"  no records in bucket {parts}")
            return
        out = {
            "bucket": list(parts),
            "n": stat.n,
            "ewma_mean_log": stat.ewma_mean,
            "correction_c": inv_log_residual(stat.ewma_mean),
            "std_log": stat.std(),
            "std_c": inv_log_residual(stat.std()),
        }
        click.echo(json.dumps(out, indent=2) if as_json else
    ...
    from coolstep.core.residual_meta import inv_log_residual
    rows = []
    for key, stat in bank.stats.items():
        rows.append({
            "bucket": list(key),
            "n": stat.n,
            "correction_c": inv_log_residual(stat.ewma_mean),
    ...
    # Human table.  bucket key is (load_band, slope_sign, accel_sign, profile_band).
    click.echo(f"{'bucket':<22} {'n':>8} {'correction':>12} {'σ':>10}")
    click.echo("-" * 56)
    for r in rows:
        click.echo(
            f"{str(tuple(r['bucket'])):<22} "
            f"{r['n']:>8.1f} "
            f"{r['correction_c']:>+11.2f}°C "
            f"{r['std_c']:>+9.2f}°C"
        )


@main.command("predict-replay")
@click.option("--since", default="1h", help="Time window (e.g. 1h, 24h).")
@click.option("--as-json", "as_json", is_flag=True, help="Emit JSON instead of human table.")
def predict_replay(since: str, as_json: bool) -> None:
    """Replay recent residuals: per-bucket stats over the time window.

    Streams ResidualLog.iter_all() but filters to records newer than
    `since` (parsed like the dashboard's window syntax).  Computes the
    same stats `predict-debug` does, restricted to the window.  Useful
    for "how is the predictor doing in the last hour vs lifetime?"
    """
    from coolstep.core.residual_log import ResidualLog
    from coolstep.core.residual_meta import ResidualBank, inv_log_residual
    ...
    for rec in log.iter_all():
        if rec.ts < cutoff:
            continue
        if rec.bucket_key is None or len(rec.bucket_key) != 4:
            continue
        bank.observe_at_bucket(tuple(rec.bucket_key), rec.residual_c)  # type: ignore[arg-type]
        total += 1
    if not bank.stats:
        click.echo(f"(no residuals in last {since})")
        return
    ...
    if as_json:
        click.echo(json.dumps({"window": since, "total": total, "buckets": rows}, indent=2))
        return
    click.echo(f"window: {since} ({total} residuals)")
    click.echo(f"{'bucket':<22} {'n':>8} {'correction':>12} {'σ':>10}")
    click.echo("-" * 56)
    for r in rows:
        click.echo(
...
def _parse_window(s: str) -> float:
    """Window parser — same shape as dashboard/server.py:_parse_window.
    Accepts e.g. '30s', '5m', '1h', '24h', '7d'.  Returns seconds."""
    s = s.strip()
    if not s:
        raise click.UsageError("empty window")
    unit_map = {"s": 1, "m": 60, "h": 3600, "d": 86400}
    if s[-1] in unit_map:
        try:
            return float(s[:-1]) * unit_map[s[-1]]
        except ValueError:
            raise click.UsageError(f"can't parse window {s!r}")
    try:
        return float(s)
    except ValueError:
        raise click.UsageError(f"can't parse window {s!r}")
