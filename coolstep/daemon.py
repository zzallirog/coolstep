...
_CONTROL_VERBS: frozenset[ActionVerb] = frozenset({
    ActionVerb.RAMP_COOLING,
    ActionVerb.REDUCE_NOISE,
    ActionVerb.CAP_BOOST,
    ActionVerb.SHIFT_POWER_ENVELOPE,
})
...
class Daemon:
    def __init__(
        ...
        # busy_ratio = tick_total_ms / (period_sec * 1000). >0.5 sustained
        # means daemon is eating its own budget — predictor sees noise from
        # its own load. Ring is 60-sample (12s @ 5Hz / 6s @ 10Hz) — short
        # enough to react, long enough to absorb single-tick spikes.
        self._tick_total_ms_ring: deque[float] = deque(maxlen=60)
        self._busy_ratio_ewma: float = 0.0
        self._busy_ratio_alpha: float = 0.1
        self._slow_tick_threshold_ms: float = 500.0
        # Balance-plan step III: hot-state mmap writer in tmpfs.
        # Per-tick fixed 64-byte struct write (~50ns) replaces ml-state.json
        # as the live-telemetry source. JSON dump kept as periodic snapshot.
        # Writer setup is fault-tolerant — if tmpfs is unavailable (CI,
        # rootless container) the daemon proceeds without a hot path.
        try:
            from coolstep.core.hot_state import Writer as _HotStateWriter
            self._hot_state_writer: Any = _HotStateWriter()
        except (OSError, PermissionError) as exc:
            log.warning("hot_state writer disabled: %r", exc)
            self._hot_state_writer = None
        # Auto-retry: a transient setup failure (e.g. tmpfs not mounted yet
        # at unit-start time, or RuntimeDirectory drop-in landed after the
        # process forked) shouldn't leave the writer permanently None.
        # The tick loop retries setup every N ticks with exponential
        # back-off, capped — by 30s real wall the writer either works or
        # we've stopped caring.
        self._hot_state_retry_ticks_remaining: int = 0
        self._hot_state_retry_backoff_ticks: int = 30  # ~6s @ 5Hz
        ...
        # Pending predictions are validated when their horizon elapses.
        self._pending_predictions: list[_PendingPrediction] = []
        # Control windows mark validated residuals whose horizon overlapped
        # a real actuator write. These records remain visible in logs/UI but
        # do not train ResidualBank's passive correction layer.
        self._intervention_windows: deque[_InterventionWindow] = deque(maxlen=512)
    ...
    async def _tick(self) -> None:
        ...
        if prediction.expected_temp_c is not None:
            ...
            self._pending_predictions.append(_PendingPrediction(
                predicted_at=frame.timestamp,
                predicted_temp_c=float(prediction.expected_temp_c),
        ...
        actions = self.decision.decide(
            ...
            cpu_temp_slope_c_per_sec=(
                float(features["cpu_temp_slope_per_sec_short"])
                if isinstance(features.get("cpu_temp_slope_per_sec_short"), (int, float))
                else None
            ),
        )
        _mark("after_decide")
        ...
        curve_ctx_payload = {
            ...
            # Short-window derivative drives the reactive `slope_preload`
            # policy — fires independent of the KNN predictor.
        ...
        # Balance-plan step III: hot-state mmap write FIRST. This is the
        # source of truth for live-telemetry; dashboard reads from tmpfs.
        # ~50ns per call vs ~50-2000ms for `_dump_ml_state` under load —
        # if the JSON dump times out or contends on disk, the hot path
        # stays current.
        if self._hot_state_writer is None and self._hot_state_retry_ticks_remaining <= 0:
            # Periodic re-init attempt — the unit may have come up before
            # tmpfs / RuntimeDirectory was ready; try again, back off on
            # repeated failure, eventually settle (capped at 5 minutes).
            try:
                from coolstep.core.hot_state import Writer as _HSW
                self._hot_state_writer = _HSW()
                log.info("hot_state writer reinitialised — back on the hot path")
                self._hot_state_retry_backoff_ticks = 30
            except (OSError, PermissionError) as exc:
                self._hot_state_retry_ticks_remaining = self._hot_state_retry_backoff_ticks
                self._hot_state_retry_backoff_ticks = min(
                    self._hot_state_retry_backoff_ticks * 2, 1500,  # 5min @ 5Hz
                )
                log.debug("hot_state retry failed: %r — next in %d ticks",
                          exc, self._hot_state_retry_ticks_remaining)
        else:
            self._hot_state_retry_ticks_remaining = max(
                0, self._hot_state_retry_ticks_remaining - 1,
            )
        if self._hot_state_writer is not None:
            try:
                from coolstep.core.hot_state import HotState
                fan_max = None
                if getattr(frame, "fans", None):
                    rpms = [f.rpm for f in frame.fans if f.rpm is not None]
                    fan_max = max(rpms) if rpms else None
                gpu_max = None
                if getattr(frame, "gpus", None):
                    gtemps = [g.temp_c for g in frame.gpus if g.temp_c is not None]
                    gpu_max = max(gtemps) if gtemps else None
                self._hot_state_writer.write(HotState(
                    ts_sec=time.time(),
                    cpu_temp_now=cpu_temp_now,
                    gpu_temp_max=gpu_max,
                    fan_max_rpm=fan_max,
                    throttle_prob=getattr(prediction, "throttle_prob", None),
                    expected_temp_c=getattr(prediction, "expected_temp_c", None),
                    busy_ratio_ewma=self._busy_ratio_ewma,
                ))
            except Exception as exc:  # noqa: BLE001
                # Hot path must never crash the tick. Disable on first failure.
                log.warning("hot_state write failed: %r — disabling", exc)
                try:
                    self._hot_state_writer.close()
                except Exception:  # noqa: BLE001
                    pass
                self._hot_state_writer = None
        _mark("hot_state")
        ...
        # Self-monitor: feed busy_ratio EWMA + ring before any branching.
        busy_ratio = tick_total_ms / (self.period * 1000.0)
        self._busy_ratio_ewma = (
            self._busy_ratio_alpha * busy_ratio
            + (1.0 - self._busy_ratio_alpha) * self._busy_ratio_ewma
        )
        self._tick_total_ms_ring.append(tick_total_ms)
        if tick_total_ms > self._slow_tick_threshold_ms:
            log.warning(
    ...
    def _do_apply(self, actuator: Actuator, action: Action) -> ActionResult | None:
        ...
        if (
            ...
            self._record_intervention_window(actuator.name, action, now=now)
    ...
    def _record_intervention_window(
        self,
        actuator_name: str,
        action: Action,
        *,
        ...
        if action.verb not in _CONTROL_VERBS:
            return
        self._intervention_windows.append(_InterventionWindow(
            started_at=float(now),
            ended_at=float(action.expires_at) + TTL_GRACE_SEC,
            verb=action.verb.value,
            actuator=actuator_name,
        ))

    def _interventions_overlapping(self, start: float, end: float) -> tuple[str, ...]:
        """Return control verbs whose active window overlaps [start, end]."""
        if not self._intervention_windows:
            return ()
        cutoff = float(start) - LOOKAHEAD_SEC * 2.0
        while (
            self._intervention_windows
            and self._intervention_windows[0].ended_at < cutoff
        ):
            self._intervention_windows.popleft()
        verbs = {
            window.verb
            for window in self._intervention_windows
            if window.started_at <= end and window.ended_at >= start
        }
    ...
    def _validate_pending_predictions(
        ...
        idx = 0
        for pending in self._pending_predictions:
            if now - pending.predicted_at < horizon:
                break  # everything past this is too young; FIFO is chronological
            try:
                residual = float(current_actual) - pending.predicted_temp_c
                intervention_verbs = self._interventions_overlapping(
                    pending.predicted_at,
                    now,
                )
                record = ResidualRecord.from_pair(
                    ...
                    horizon_sec=horizon,
                    predicted_temp_c=pending.predicted_temp_c,
                    actual_temp_c=float(current_actual),
                    model_name=pending.model_name,
                    profile=self._active_tuned_profile(),
                    bucket_key=pending.bucket_key,
                    features=pending.features,
                    intervened=bool(intervention_verbs),
                    intervention_verbs=intervention_verbs,
                )
                ...
                # Controlled residuals are deliberately excluded: an actuator
                # overlapping the horizon changed the outcome, so the sample
                # is no longer passive thermal behaviour.
                if (
                    isinstance(self.predictor, MetaPredictor)
                    and pending.bucket_key is not None
                    and not intervention_verbs
                ):
                    self.predictor.bank.observe_at_bucket(
                        tuple(pending.bucket_key),  # type: ignore[arg-type]
                        residual,
                    )
                ...
                try:
                    spike_record = self.spike_detector.update(
                        ...
                        features=pending.features,
                        predictor_model=pending.model_name,
                    )
    ...
    def _recover_armed_actions(self) -> None:
        ...
        for entry in armed:
            ...
            if not name or expires_at >= now + 60.0:
                ...
                action = Action(verb=verb, params={}, expires_at=expires_at)
                self._armed_actions[name] = (action, applied_at)
                self._record_intervention_window(str(name), action, now=applied_at)
                continue
            actuator = self._find_actuator(name)
            if actuator is None:
                log.info(
    ...
    def _dump_ml_state(self, prediction, features: dict[str, float]) -> None:  # type: ignore[no-untyped-def]
        ...
        # Balance-plan step I: self-monitor signals. Reserved namespace
        # `self_monitor.*` — step II grows this with PSI / memory / OOM
        # counters read from systemd + cgroup. p95 uses sorted-index on
        # a 60-element ring (O(n log n) but trivial at this size).
        ring = list(self._tick_total_ms_ring)
        if ring:
            ring_sorted = sorted(ring)
            p95_idx = max(0, int(round(len(ring_sorted) * 0.95)) - 1)
            tick_p95_ms = ring_sorted[p95_idx]
            slow_count = sum(
                1 for v in ring if v > self._slow_tick_threshold_ms
            )
        else:
            tick_p95_ms = 0.0
            slow_count = 0
        snapshot["self_monitor"] = {
            "busy_ratio_ewma": round(self._busy_ratio_ewma, 4),
            "busy_ratio_p95_60_ticks": round(
                tick_p95_ms / (self.period * 1000.0), 4
            ),
            "tick_total_ms_p95_60_ticks": round(tick_p95_ms, 1),
            "slow_tick_count_60_ticks": slow_count,
            "slow_tick_threshold_ms": self._slow_tick_threshold_ms,
