...
from coolstep.core.predictor import AlwaysIdleBaseline, KnnPredictor
...
class Daemon:
    def __init__(
        ...
        if self.chroma.available:
            self.predictor = KnnPredictor(self.embedder, self.chroma)
        else:
            self.predictor = AlwaysIdleBaseline()
