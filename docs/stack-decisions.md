...
## ADR-008: SSE, not WebSocket

**Decision:** live updates use Server-Sent Events.

**Why:** the dashboard is read-only. We never send messages from
browser to server outside REST calls. SSE handles the one direction we
need with auto-reconnect built into `EventSource`, no upgrade
handshake, no WS frame plumbing. Proxies (nginx, traefik, even
`gh tunnel`) deal with SSE more reliably than with WebSocket.

**When to revisit:** if a future feature actually needs full-duplex
(live actuator override from the dashboard?), we'd upgrade specific
endpoints, not the whole stack.
