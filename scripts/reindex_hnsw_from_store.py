...
from coolstep.core import fingerprint as fp  # noqa: E402
...
def main() -> int:
    ...
    # Sliding window for trajectory features (slope/accel).  15 frames ≈ 15s
    # at 1 Hz — matches the daemon's short-slope window.  Frames with large
    # timestamp gaps (>30s) reset the window to avoid cross-session slopes.
    _WINDOW_SIZE = 15
    _MAX_GAP_S = 30.0
    window_buf: deque[object] = deque(maxlen=_WINDOW_SIZE)
    ...
    for frame in frames:
        ...
        if window_buf:
            prev_ts = window_buf[-1].timestamp  # type: ignore[attr-defined]
            if frame.timestamp - prev_ts > _MAX_GAP_S:
                window_buf.clear()
        window_buf.append(frame)

        wf = fp.extract(list(window_buf)) if len(window_buf) >= 2 else None
        vector = embedder.embed(frame, wf)
        if vector is None:
            skipped_no_vec += 1
            continue
